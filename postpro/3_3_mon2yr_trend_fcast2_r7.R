...
# fname='_fcast2_r4'
# fname='_fcast2_r5'
fname='_fcast2_r7'  # r456 has different number of columns
moy <- rep(c(1:12),8)
year<- rep(c(2011:2018),each=12)

annplt <- matrix(NA,ncol = 67,nrow = 48)
annsdplt<- matrix(NA,ncol = 67,nrow = 48)
...
for(i in 1:6){
  ...
  # QCout13-17 is sum of monthly Rb, average with dom:
  df1$QCt13_mean1[df1$moy <12]<-df1$QCt13_mean1[df1$moy <12]/30
  df1$QCt13_mean1[df1$moy ==12]<-df1$QCt13_mean1[df1$moy ==12]/35
  df2$QCt13_mean1[df2$moy <12]<-df2$QCt13_mean1[df2$moy <12]/30
  df2$QCt13_mean1[df2$moy ==12]<-df2$QCt13_mean1[df2$moy ==12]/35
  ...
  df11 <- df1[c(12,24,36,48,60,72,84,96),c(18:25,53:62)] # Cpools end of year  
  dfmean9 <- df1[c(12,24,36,48,60,72,84,96)-3,c(18:25,53:62)] # Cpools end of growing season, end of September
  dfmean4 <- df1[c(12,24,36,48,60,72,84,96)-8,c(18:25,53:62)] # Cpools start of growing season, end of April
  df12 <- aggregate(df1[,c(4:17,26:52)],by=list(df1$year),FUN=sum)   # Cfluxes sum
  # df21 <- aggregate(df2[,c(18:25,42:51)],by=list(df2$year),FUN=mean) # Cpools sd
  df21 <- df2[c(12,24,36,48,60,72,84,96),c(18:25,53:62)] # Cpools sd, end of
  dfsd9 <- df2[c(12,24,36,48,60,72,84,96)-3,c(18:25,53:62)] # Cpools sd, end of growing season, end of September
  dfsd4 <- df2[c(12,24,36,48,60,72,84,96)-8,c(18:25,53:62)] # Cpools start of growing season, end of April
  df22 <- aggregate(df2[,c(4:17,26:52)],by=list(df2$year),FUN=sum)   # Cfluxes sd
  ...
  colnames(annsd)[1:16] <- c('QC1','QC2','QC3','QC4','QC5','QC6','QC7','QC8',
                           'dQC1','dQC2','dQC3','dQC4','dQC5','dQC6','dQC7','dQC8')
  if (i==1){
    annplt <- ann
    annsdplt<-annsd
