...
CS<-c('_MCMC_CS1722_arv','_MCMC_CS1423_arv','_MCMC_CS1422_arv','_MCMC_CS1623_arv','_MCMC_CS1422_arv','_MCMC_CS1622_arv')
...
for(i in 1:6){
  ...
  df11 <- aggregate(df1[,c(18:25,32:41)],by=list(df1$year),FUN=mean) # Cpools mean
  df12 <- aggregate(df1[,c(4:17,26:31)],by=list(df1$year),FUN=sum)   # Cfluxes mean
  df21 <- aggregate(df2[,c(18:25,32:41)],by=list(df2$year),FUN=mean) # Cpools sd
  df22 <- aggregate(df2[,c(4:17,26:31)],by=list(df2$year),FUN=sum)   # Cfluxes sd
  ...
  ann <- cbind(df13[,-1],df14[,-1])
  annsd<-cbind(df23[,-1],df24[,-1])
...
# save annplt
setwd("~/RESEARCH/TECO_SPRUCE_site/TECO_2.21_Dissertation/jpl_server/TECO_2.21/postpro/statistics/rowmean/")
write.csv(annplt, file = paste("3_3_mon2yr_trend_",fname,".csv",sep=''))
write.csv(annsdplt, file = paste("3_3_mon2yr_trend_sd_",fname,".csv",sep=''))


setwd("~/RESEARCH/TECO_SPRUCE_site/TECO_2.21_Dissertation/jpl_server/TECO_2.21/postpro/")
pdf(paste("3_3_figure_",fname,".pdf",sep=''),width=15.5, height=7.75, compress=FALSE)
