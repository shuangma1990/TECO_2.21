...
# fname='_fcast2_r4'
# fname='_fcast2_r5'
fname='_fcast2_r6'  # r456 has different number of columns
moy <- rep(c(1:12),8)
year<- rep(c(2011:2018),each=12)

annplt <- matrix(NA,ncol = 62,nrow = 48)
annsdplt<- matrix(NA,ncol = 62,nrow = 48)
...
for(i in 1:6){
  ...
  df11 <- df1[c(12,24,36,48,60,72,84,96),c(18:25,48:57)] # Cpools end of year  
  dfmean9 <- df1[c(12,24,36,48,60,72,84,96)-3,c(18:25,48:57)] # Cpools end of growing season, end of September
  dfmean4 <- df1[c(12,24,36,48,60,72,84,96)-8,c(18:25,48:57)] # Cpools start of growing season, end of April
  df12 <- aggregate(df1[,c(4:17,26:47)],by=list(df1$year),FUN=sum)   # Cfluxes sum
  # df21 <- aggregate(df2[,c(18:25,42:51)],by=list(df2$year),FUN=mean) # Cpools sd
  df21 <- df2[c(12,24,36,48,60,72,84,96),c(18:25,48:57)] # Cpools sd, end of
  dfsd9 <- df2[c(12,24,36,48,60,72,84,96)-3,c(18:25,48:57)] # Cpools sd, end of growing season, end of September
  dfsd4 <- df2[c(12,24,36,48,60,72,84,96)-8,c(18:25,48:57)] # Cpools start of growing season, end of April
  df22 <- aggregate(df2[,c(4:17,26:47)],by=list(df2$year),FUN=sum)   # Cfluxes sd
