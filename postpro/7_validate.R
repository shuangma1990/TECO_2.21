...
pdf(paste("7_validate",fname,".pdf",sep=''),width=4., height=3.75, compress=FALSE)
# use MAT for only moss and shrub, these were ascended 
MAT=df$moss_ANPP
xlimv=c(3,18)
par(mar=c(0.5,0.,0,0.5),mfrow=c(3,2),oma=c(3,4,1,0))
av=0.8
pv=1.
lgv=0.8

# CO2 AND CH4
plot(df$Rh_co2[1:6],df$Rh_co2_y[1:6],col='black',xlim = xlimv,ylim = c(-1000,100),pch=16,cex=pv,xaxt="n",yaxt="n")# obs
...
points(df$Rh_co2[7:36],df$Rh_co2_y[7:36],col='black',pch=16,cex=pv)
...
points(df$ch4[1:6],df$ch4_y[1:6],col='grey',xlim = xlimv,ylim = c(-600,0),pch=16,cex=pv,xaxt="n",yaxt="n")# obs
...
legend("bottomleft",inset=0.02,legend=c(expression('Observed CH'[4]),expression('Modeled CH'[4]),
                                        expression('Observed Rh CO'[2]),expression('Modeled Rh CO'[2])),
       ncol=1,col=c('grey','#404788ff','black','#1f968bff'),x.intersp=0.6,y.intersp=1.,pch=c(16,17,16,17),cex=lgv,box.lty=0)
mtext('(a)',side=3,at=3.2,line=-1.1,cex=0.8)
# NEE
plot(df$OBSnee,df$OBSnee_y,col='black',xlim = xlimv,ylim = c(-1000,100),pch=16,cex=pv,xaxt="n",yaxt="n")# obs
axis(side=1, tck=0.03, cex.axis=1, mgp=c(3,0.3,0),at=c(5,10,15),labels=F)
axis(side=2, labels=F, at=c(0,-400,-800),tck=0.03, cex.axis=1, mgp=c(3,0.3,0),las=2)
points(df$ELM_NEE,df$ELM_NEE_y,col='grey',pch=15,cex=pv)
...
legend("bottomleft",inset=0.02,legend=c(expression('Observed NEE'),expression('Modeled NEE'),'ELM Modeled NEE'),
       ncol=1,col=c('black','#55c667ff','grey'),pch=c(16,17,15),x.intersp=0.6,y.intersp=1.,cex=lgv,box.lty=0)
mtext('(b)',side=3,at=3.2,line=-1.1,cex=0.8)
...
mtext('(c)',side=3,at=3.2,line=-1.1,cex=0.8)
# woodc
plot(xair,woodc,col='grey',xlim = xlimv,pch=16,ylim=c(0,600),cex=pv,xaxt="n",yaxt="n")# obs
axis(side=1, tck=0.03, cex.axis=1, mgp=c(3,0.3,0),labels=F)
axis(side=2, labels=F, at=c(0,200,400,600),tck=0.03, cex.axis=1, mgp=c(3,0.3,0),las=2)
...
mtext('(d)',side=3,at=3.2,line=-1.1,cex=0.8)

# ANPP+BNPP
y1=df$moss_ANPP_y+df$shrub_ANPP_y
y2=df$moss_ANPP_y+df$shrub_ANPP_y+df$fineroot_BNPP_y
plot(MAT,y2,col='black',xlim = xlimv,pch=16,ylim=c(0,600),cex=pv,xaxt="n",yaxt="n")# obs
axis(side=1, tck=0.03, cex.axis=1, mgp=c(3,0.3,0),labels=F)
axis(side=2, labels=T, at=c(0,200,400,600),tck=0.03, cex.axis=1, mgp=c(3,0.3,0),las=2)
...
# BNPP
points(df$fineroot_BNPP,df$fineroot_BNPP_y,col='grey',xlim = xlimv,pch=16,ylim=c(0,600),cex=pv,xaxt="n",yaxt="n")# obs
...
legend("topright",inset=0.02,legend=c('Observed NPP','Modeled NPP','Observed BNPP','Modeled BNPP'),ncol=2,
       col=c('black','#f68f46ff','grey','#cc6a70ff'),x.intersp=0.6,y.intersp=1.,pch=c(16,17,16,17),cex=lgv,box.lty=0)
mtext('(e)',side=3,at=3.2,line=-1.1,cex=0.8)
# ANPP
...
mtext('(f)',side=3,at=3.2,line=-1.1,cex=0.8)
# # BNPP
