...
setwd("~/RESEARCH/TECO_SPRUCE_site/TECO_2.21_Dissertation/jpl_server/TECO_2.21/postpro/")
pdf(paste("3_3_yr_trend_final_f1.pdf",sep=''),width=7.5, height=4., compress=FALSE)
...
m=0
for (c in c(27,30,29,32,31,28)){
  # c=21
  m=m+1
  op <- par(mar=c(0., 3.5, 0.5, 0))
  x <- xair
  if ((c==27)|(c==28)){
  ...
  axis(side=2, labels=T, tck=0.03, las=1,mgp=c(3, 0.3,0), at=c(seq(from=yrange[1],to=yrange[2],by=200)))
  if (m>3){
  ...
  # mtext(paste('(',letter[m],')',sep=''),outer=FALSE,side=1,line = -5.,col = "black",
  #       at=par("usr")[1]+0.9*diff(par("usr")[1:2]),cex=rsize)
  # ************************************************

    # ************************************************
  points(xair,yvalue,col=cab,lwd=3 ,pch=16, cex=pointcex, ylim=c(0,1000), xlim=xrg,main=NA)
  model3 <- lm(yvalue~ xair)
  summary(model3)
  newdata <- data.frame(xair=xairseq)
  ...
  #-----  OLS  ----------------------------------------------------------------
  x0 <- seq(min(xair), max(xair), length = 20)  ## prediction grid
  y0 <- predict.lm(model3, newdata = list(xair = x0))  ## predicted values
  ...
  mtext(side=2, text=labv[m], line=2, cex=rsize)
  mtext(side=3,text=letter[m],line=-2,cex=rsize,at=1)
  box()
}
